...
// Declare insert/erase as taking const& to match Rust's &self call site via cxx
bool insert(const ConcurrentHashMapU64Opaque& map, uint64_t key, uint64_t value);
uint64_t find(const ConcurrentHashMapU64Opaque& map, uint64_t key); // Returns value or sentinel
size_t erase(const ConcurrentHashMapU64Opaque& map, uint64_t key); // Returns number erased
